    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                img = torch.from_numpy(self.vae_decoder(denoised / 0.18215, share_inputs=True, share_outputs=True)[0])
                do_denormalize = [True] * img.shape[0]
                img = self.image_processor.postprocess(img, output_type=output_type, do_denormalize=do_denormalize)
        ...
        return StableDiffusionPipelineOutput(images=img_history, nsfw_content_detected=has_nsfw_concept)